...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(cn_dsi==3 && m_dsi>range0 && m_dsi<range1 && pst_dsi>2.5 && TMath::Abs(helicity)>0.0 && TMath::Prob(chi2_dsi, dgf_dsi)>0.001 && TMath::Prob(chi2_kst, dgf_kst)>0.1)){
    mass->setVal(m_dsi);
