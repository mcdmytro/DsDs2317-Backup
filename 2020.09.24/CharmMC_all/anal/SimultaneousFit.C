...
void SimultaneousFit(){
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if((m_ds17-m_dsii)>r0_2317 && (m_ds17-m_dsii)<r1_2317 && TMath::Abs(hel_phi)>0.35 && cn_dsi==1 && cn_dsii==1 && pst_dsii>2.5 && pst_ds17>3.5 && TMath::Prob(c2_ds17, dgf_ds17)>0.001 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01){
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if((m_ds60-m_dsst)>r0_2460 && (m_ds60-m_dsst)<r1_2460 && TMath::Abs(hel_phi)>0.35 && cn_dsw==1 && cn_dsn==1 && pst_dsn>2.5 && pst_ds60>3.5 && TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001){
  ...
  RooRealVar *mean_2317        = new RooRealVar("sgn mean", "sgn mean", 0.34897, 0.2, 0.5);
  RooRealVar *mean_g2_2317     = new RooRealVar("refl mean", "refl mean", 0.35, 0.33, 0.37);
  RooRealVar *sigma_gaus1_2317 = new RooRealVar("sgn sigma", "Sigma of gaussian",0.0047, 0.003,0.009);
  RooRealVar *sigma_gaus2_2317 = new RooRealVar("refl sigma", "Sigma of gaussian",0.01330, 0.01,0.02);
  ...
  RooRealVar *sig_2317        = new RooRealVar("N_sig", "N true signal", 3893, 0, 5000);
  RooRealVar *sig_2460        = new RooRealVar("N_sig", "N true signal", 1300, 0, 2700);
  RooRealVar *refl_2317       = new RooRealVar("Nrefl", "Nrefl", sig_2460.getVal()*1.94148);  
  ...
  RooRealVar *mean_2460        = new RooRealVar("sgn mean", "sgn mean", 0.3473, 0.2, 0.5);
  RooRealVar *mean_g2_2460     = new RooRealVar("BS mean", "Mean of gaussian", 0.3425, 0.2, 0.5);
  RooRealVar *mean_g3_2460     = new RooRealVar("refl mean", "Mean of gaussian", 0.35, 0.33, 0.37);

  RooRealVar *sigma_gaus1_2460 = new RooRealVar("sgn sigma", "Sigma of gaussian",0.004, 0.003, 0.009);
  RooRealVar *sigma_gaus2_2460 = new RooRealVar("BS sigma", "Sigma of gaussian",0.0137, 0.012, 0.24);
  RooRealVar *sigma_gaus3_2460 = new RooRealVar("refl sigma", "Sigma of gaussian",0.0151, 0.01, 0.015);
  ...
  RooRealVar *broken_sig_2460 = new RooRealVar("N_BS", "N NS", sig_2460.getVal()*0.62712);
  RooRealVar *refl_2460       = new RooRealVar("Nrefl", "Nrefl", sig_2317.getVal()*0.251142);
  ...
  RooAddPdf *pdf_2460 = new RooAddPdf ("pdf_2460", "Gaussian + Pol",RooArgList(*gaus1_2460, *gaus2_2460, *gaus3_2460, *pol_2460), RooArgList(*sig_2460, *broken_sig_2460, *refl_2460, *bkg_pol_2460));
  ...
  */
  ...
  cv->SaveAs("SimultaneousFit_CMC.pdf","pdf");
