...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(m_phi2>range0 && m_phi2<range1 && TMath::Prob(chi2_phi2, dgf_phi2)>0.001){
    mass->setVal(m_phi2);
  ...
  RooRealVar *sigma=new RooRealVar("sigma", "Sigma of gaussian",0.00052,0.0003,0.015,"GeV");
  ...
  RooRealVar *GZ = new RooRealVar("GZ","Z width", 0.00441, 0.003, 0.006, "GeV");
  ...
  RooPlot *frame = mass->frame(Title("#varphi mass"));
  data->plotOn(frame);
  ...
  cv->SaveAs("CMC_phi_MassAndPull.pdf","pdf");
}
