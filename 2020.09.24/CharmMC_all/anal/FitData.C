...
chain.Add("../BCS.root");
 ...
 Float_t mctruth, cos, R2, helicity,pst_dsi, m_dsi, mc_dsi, dgf_dsi, chi2_dsi, cn_dsi, pst_ds17, m_ds17, dgf_ds17, chi2_ds17,pst_dsii, m_dsii, dgf_dsii, chi2_dsii, cn_dsii, ppi, dgf_pi, chi2_pi, dgf_phi1, chi2_phi1, dgf_phi2, chi2_phi2, chi2_ipi, dgf_ipi, pipi, chi2_iipi, dgf_iipi, piipi, k1_id, k2_id, k3_id, k4_id, pi1_id, pi2_id, mc_ds17, mc_dsi;
...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(m_dsi>range0 && m_dsi<range1 && pst_dsi>2.5 && TMath::Prob(chi2_dsi, dgf_dsi)>0.001){// && TMath::Abs(mc_dsi)==431){
    mass->setVal(m_dsi);
