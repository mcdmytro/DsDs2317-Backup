...
Float_t range0=1.9684895, range1=1.9684905;
...
Float_t ds_gm;
chain.SetBranchAddress("ds_gm", &ds_gm);

RooDataSet *data = new RooDataSet("data","", RooArgSet(*x),"GeV");

for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(ds_gm<range1 && ds_gm>range0){
    x->setVal(ds_gm);
...
cv->SaveAs("../pdf_files/ds_mass_gen.pdf","pdf");
cv->SaveAs("../eps_files/ds_mass_gen.eps","eps");
}
