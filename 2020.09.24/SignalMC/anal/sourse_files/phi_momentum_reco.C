...
void phi_momentum_reco(){
gROOT->Reset();
gSystem->Load("libRooFit");

Float_t range0=0.3, range1=1.5;
...
Float_t p_phi1;
chain.SetBranchAddress("p_phi1", &p_phi1);

RooDataSet *data = new RooDataSet("data","", RooArgSet(*x),"GeV");

for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(p_phi1<range1 && p_phi1>range0){
    x->setVal(p_phi1);
...
cv->SaveAs("../pdf_files/phi_momentum_reco.pdf","pdf");
cv->SaveAs("../eps_files/phi_momentum_reco.eps","eps");
}
