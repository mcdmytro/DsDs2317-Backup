...
void ds2317_energy_reco(){
gROOT->Reset();
gSystem->Load("libRooFit");

Float_t range0=2.2, range1=3;
...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(e_ds17<range1 && e_ds17>range0){
    x->setVal(e_ds17);
...
frame->GetXaxis()->SetTitle("E(D_{s}#pi^{0}), (GeV/c)");
...
cv->SaveAs("../pdf_files/ds2317_energy_reco.pdf","pdf");
cv->SaveAs("../eps_files/ds2317_energy_reco.eps","eps");
}
