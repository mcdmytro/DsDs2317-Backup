...
RooDataSet *seldata = new RooDataSet("seldata","", RooArgSet(*pst),"GeV");
 ...
 for (int i=0; i<chain.GetEntries();i++){
   chain.GetEntry(i);
   if(pst_dsp<range1 && pst_dsp>range0 && TMath::Abs(hel_phi)>0.35 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2){
     pst->setVal(pst_dsp);
     data->add(RooArgSet(*pst));
   }}
 for (int i=0; i<chain.GetEntries();i++){
   chain.GetEntry(i);
   if(pst_dsp<range1 && pst_dsp>range0 && pst_ds17>3.5 && TMath::Abs(hel_phi)>0.35 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2){
     pst->setVal(pst_dsp);
  ...
  */
  ...
  cv->SaveAs("Data_DsDs2317_pst_overlaped.pdf","pdf");
