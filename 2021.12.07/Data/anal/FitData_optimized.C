...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(m_dsi>range0 && m_dsi<range1 && pst_ds17>2.79 && TMath::Abs(hel_phi)>0.44 && TMath::Prob(c2_ds17, dgf_ds17)>0.001 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsi!=2 && cn_dsii!=2){
    mass->setVal(m_dsi);
  ...
  */
  ...
  cv->SaveAs("Data_Ds_optimized.pdf","pdf");
}
