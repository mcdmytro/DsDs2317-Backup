...
void Fit_Dsstar_massDelta(){
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if((m_dsst-m_dsn)>range0 && (m_dsst-m_dsn)<range1 && pst_ds60>3.5 && TMath::Abs(hel_phi)>0.35 && cn_dsn!=2 & TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001){
      mass->setVal(m_dsst-m_dsn);
      data->add(RooArgSet(*mass));
    }}
  
  RooRealVar *mean  = new RooRealVar("mean", "sgn mean", 0.14, 0.08, 0.2);
  ...
  */
  ...
  cv->SaveAs("Data_Dsst_delta_ready.pdf","pdf");
  fitresult->Print("v");
}
