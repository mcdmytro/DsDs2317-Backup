...
Float_t r2, m_dsp, pst_dsp, mctruth, pst_dsi, m_dsi, mc_dsi, dgf_dsi, c2_dsi, cn_dsi, pst_ds17, m_ds17, dgf_ds17, c2_ds17,pst_dsii, m_dsii, dgf_dsii, c2_dsii, cn_dsii, ppi, dgf_pi0, c2_pi0, dgf_phi1, c2_phi1, dgf_phi2, c2_phi2, c2_ipi, dgf_ipi, pipi, c2_iipi, dgf_iipi, piipi, mc_ds17, mc_dsii, hel_phi;
...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(m_dsi>range0 && m_dsi<range1 && pst_ds17>2.79 && TMath::Abs(hel_phi)>0.44 && cn_dsi!=2 && cn_dsii!=2){// && TMath::Abs(mc_dsi)==431){
    mass->setVal(m_dsi);
  ...
  frame->GetYaxis()->SetTitle("Events / 1.5 MeV");
  ...
  */
  ...
  cv->SaveAs("Data_Ds_optimized_ready.pdf","pdf");
}
