...
void Delta_2460(){
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if((m_ds60-m_dsst)>range0 && (m_ds60-m_dsst)<range1 && pst_ds60>3.5 && TMath::Abs(hel_phi)>0.35 && cn_dsn!=2 && TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001){
  ...
  RooRealVar *sigma_gS  = new RooRealVar("sgn sigma",  "Sigma of gaussian", 0.006, 0.003, 0.02);
