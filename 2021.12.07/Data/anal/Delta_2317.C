...
void Delta_2317(){
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if((m_ds17-m_dsii)>range0 && (m_ds17-m_dsii)<range1 && pst_ds17>3.5 && TMath::Abs(hel_phi)>0.35 && TMath::Prob(c2_ds17, dgf_ds17)>0.001 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2){
  ...
  */
  ...
  cv->SaveAs("Data_Ds2317_delta_ready.pdf","pdf");
  fitresult->Print("v");
}
