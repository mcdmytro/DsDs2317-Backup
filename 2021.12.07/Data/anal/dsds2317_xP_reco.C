...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  xP=pst_dsp/TMath::Sqrt(ecm*ecm-m_dsp*m_dsp);
  if(xP<range1 && xP>range0 && TMath::Abs(hel_phi)>0.35 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2){
    x->setVal(xP);
...
cv->SaveAs("Data_dsds2317_xP.pdf","pdf");
