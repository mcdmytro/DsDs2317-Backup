...
for (int i=0; i<chain.GetEntries();i++){
  ...
  chain.GetEntry(i);
  if(m_var>range0 && m_var<range1 && m_ds17>x0_ds17 && m_ds17<x1_ds17 && pst_ds17>2.83 && TMath::Abs(hel_phi)>0.35 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2)){
    mass->setVal(m_var);
 ...
 */
 ...
 cv->SaveAs("Data_DspinDs2317_subtracted_2.83.pdf","pdf");
  fitresult->Print("v");
}
