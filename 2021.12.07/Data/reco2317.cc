...
namespace Belle {
  ...
  void User_reco::hist_def( void )
  {
    ...
    std::string rs=t_par("DsP")+t_par("Ds17")+t_par("DsI")+t_par("DsII")+t_par("phi1")+t_par("phi2")+t_par("pi1")+t_par("pi2")+t_par("ipi")+t_par("iipi")+t_par("kst0")+t_par("pi0")+t_par("K1")+t_par("K2")+t_par("K3")+t_par("K4")+"egam1 egam2 egam3 egam4 egam5 egam6 ip_x ip_y ip_z mctr cos0 thrr thrs r2 DsPart count hel k1_id k2_id pi1_id k3_id k4_id pi2_id cosgg cosdspi0 cos17ds hel_phi hel_ds hel_ds17 x_p ecm mult_b mult_e";
...
*/
  ...
  if(DSPRT.size()!=0){
    int i=0;
    UserInfo & info_ds17 =dynamic_cast<UserInfo&>(DSPRT[i].child(0).userInfo());
    UserInfo & info_pi0 =dynamic_cast<UserInfo&>(DSPRT[i].child(0).child(1).userInfo());
    UserInfo & info_dsII =dynamic_cast<UserInfo&>(DSPRT[i].child(0).child(0).userInfo());
    UserInfo & info_phiII =dynamic_cast<UserInfo&>(DSPRT[i].child(0).child(0).child(0).userInfo());
    UserInfo & info_dsI =dynamic_cast<UserInfo&>(DSPRT[i].child(1).userInfo());
    UserInfo & info_phiI =dynamic_cast<UserInfo&>(DSPRT[i].child(1).child(0).userInfo());
    del_min = ((info_ds17.vchisq())*(info_ds17.vchisq())+(info_pi0.vchisq())*(info_pi0.vchisq())+(info_dsII.vchisq())*(info_dsII.vchisq())+(info_phiII.vchisq())*(info_phiII.vchisq())+(info_dsI.vchisq())*(info_dsI.vchisq())+(info_phiI.vchisq())*(info_phiI.vchisq()));
  }
   if(DSPRT.size()>1){
     for (int i=1; i<DSPRT.size(); ++i){
       UserInfo & info_ds17 =dynamic_cast<UserInfo&>(DSPRT[i].child(0).userInfo());
       ...
       UserInfo & info_dsI =dynamic_cast<UserInfo&>(DSPRT[i].child(1).userInfo());
       UserInfo & info_phiI =dynamic_cast<UserInfo&>(DSPRT[i].child(1).child(0).userInfo());
       if (((info_ds17.vchisq())*(info_ds17.vchisq())+(info_pi0.vchisq())*(info_pi0.vchisq())+(info_dsII.vchisq())*(info_dsII.vchisq())+(info_phiII.vchisq())*(info_phiII.vchisq())+(info_dsI.vchisq())*(info_dsI.vchisq())+(info_phiI.vchisq())*(info_phiI.vchisq()))<del_min){
	 del_min=((info_ds17.vchisq())*(info_ds17.vchisq())+(info_pi0.vchisq())*(info_pi0.vchisq())+(info_dsII.vchisq())*(info_dsII.vchisq())+(info_phiII.vchisq())*(info_phiII.vchisq())+(info_dsI.vchisq())*(info_dsI.vchisq())+(info_phiI.vchisq())*(info_phiI.vchisq()));;
 	bst=i;}
       }}
 ...
 for(unsigned i=0;i<DSPRT.size();++i){
   ...
   for(std::vector<Mdst_charged>::iterator it=mdstchgmgr.begin();
     ...
     if(it->get_ID() == DSPRT[i].child(0).child(0).child(1).mdstCharged().get_ID()) continue;
...
//===================================================================================
  ...
  // cout << "dsii_cn=" << dsii_cn << "   dsi_cn=" << dsi_cn << endl;
    t_fill(p,t,"DsP");
