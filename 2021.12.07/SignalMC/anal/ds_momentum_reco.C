...
Float_t p_dsi;
chain.SetBranchAddress("p_dsi", &p_dsi);

RooDataSet *data = new RooDataSet("data","", RooArgSet(*x),"GeV");

for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(p_dsi<range1 && p_dsi>range0){
    x->setVal(p_dsi);
...
cv->SaveAs("../pdf_files/ds_momentum_reco.pdf","pdf");
cv->SaveAs("../pdf_files/ds_momentum_reco.eps","eps");
}
