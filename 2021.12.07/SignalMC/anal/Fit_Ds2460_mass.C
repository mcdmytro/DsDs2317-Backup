...
void Fit_Ds2460_mass(){
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if(m_ds60>range0 && m_ds60<range1 && hel_ds60<0.7 && TMath::Abs(hel_phi)>0.44 && cn_dsn!=2 && TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001){
      	mass->setVal(m_ds60);
