...
Float_t range0=1.011, range1=1.029;
//Float_t range0=2.12, range1=2.5;
Float_t x0=1.011, x1=1.029;
...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(m_phi2>range0 && m_phi2<range1){
    mass->setVal(m_phi2);
    data->add(RooArgSet(*mass));
}}


  RooRealVar *mean = new RooRealVar("mean", "Mean of gaussian",1.020,1.010,3.030,"GeV");
  RooRealVar *sigma=new RooRealVar("sigma", "Sigma of gaussian",0.005,0.0003,0.015,"GeV");
  ...
  RooRealVar *gamma = new RooRealVar("gamma","phi width", 0.00441, 0.003, 0.006, "GeV");
  RooVoigtian *voigt = new RooVoigtian("voigt","Reconstructed peak", *mass, *mean, *gamma, *sigma);

  RooRealVar *sig = new RooRealVar("Nsig", "",0.,1E5);
  RooRealVar *bkg = new RooRealVar("Nbkg", "",0.,1E5);

  RooAddPdf *pdf = new RooAddPdf ("pdf", "Voigtian + Pol",RooArgList(*voigt, *Pol), RooArgList(*sig, *bkg));
  ...
  frame->GetYaxis()->SetTitle("Entries / 0.18 MeV/c^{2} ");
  ...
  cv->SaveAs("SMC_phi_ready.pdf","pdf");

//   EXT PARAMETER                                INTERNAL      INTERNAL
// NO.   NAME      VALUE            ERROR       STEP SIZE       VALUE
//  1  Nbkg         7.63273e+02   2.16875e+02   3.93609e-03  -1.74575e+00
//  2  Nsig         2.25483e+04   2.60691e+02   1.41254e-03  -2.56038e+00
//  3  a0          -1.63434e-01   1.60169e-02   3.62900e-02  -1.63434e-03
//  4  a1           2.16606e+00   7.04963e+00   1.13564e-02   2.16623e-02
//  5  gamma        4.11100e-03   1.23328e-04   1.39629e-02  -2.62335e-01
//  6  mean         1.01945e+00   2.06258e-05   5.73655e-05  -1.43387e+00
//  7  sigma        8.40372e-04   7.69804e-05   5.09403e-03  -1.95664e+00
}
