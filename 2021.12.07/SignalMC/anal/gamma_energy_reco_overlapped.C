...
void gamma_energy_reco_overlapped(){
  gROOT->Reset();
  ...
  RooDataSet *data_notTrue = new RooDataSet("data_notTrue","", RooArgSet(*x));
  RooDataSet *data_true = new RooDataSet("data_true","", RooArgSet(*x));

  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if(egam1<range1 && egam1>range0 && TMath::Abs(mc_ds17)!=10431){
      x->setVal(egam1);
      data_notTrue->add(RooArgSet(*x));
    }}
  ...
  for (int i=0; i<chain.GetEntries();i++){
    ...
    if(egam1<range1 && egam1>range0 && TMath::Abs(mc_ds17)==10431){
      x->setVal(egam1);
      data_true->add(RooArgSet(*x));
    }}

  RooPlot *frame = x->frame(Bins(100));
  data_notTrue->plotOn(frame, Name("data_notTrue"));
  data_true->plotOn(frame, Name("data_true"), MarkerColor(kRed), LineColor(kRed));
  ...
  cv->SaveAs("./pdf_files/SMC_BCS_gamma_energy_reco_overlapped.pdf","pdf");
}
