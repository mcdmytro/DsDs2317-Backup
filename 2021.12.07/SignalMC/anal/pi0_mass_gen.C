...
Float_t range0=0.1349768, range1=0.1349772;
...
Float_t pi0_gm;
chain.SetBranchAddress("pi0_gm", &pi0_gm);

RooDataSet *data = new RooDataSet("data","", RooArgSet(*x),"GeV");

for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(pi0_gm<range1 && pi0_gm>range0){
    x->setVal(pi0_gm);
...
cv->SaveAs("../pdf_files/pi0_mass_gen.pdf","pdf");
cv->SaveAs("../eps_files/pi0_mass_gen.eps","eps");
}
