...
void Fit_Dsstar_massDelta(){
  ...
  TChain chain("h1");
  chain.Add("../Ds2460inDs2460_BCS_noDsstKmvFit.root");
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if((m_dsst-m_dsn)>range0 && (m_dsst-m_dsn)<range1&& hel_ds60<0.7 && TMath::Abs(hel_phi)>0.44 && cn_dsw!=2 && cn_dsn!=2 && TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001){
  ...
  data->plotOn(frame);
  frame->SetTitle("");
  frame->GetXaxis()->SetTitle("M(D_{s}#gamma)-M(D_{s}) [GeV/c^{2}] ");
  ...
  frame->GetYaxis()->SetTitle("Events / 1.2 keV/c^{2}");
  ...
  */
  ...
  cv->SaveAs("./pdf_files/SMC_BCS_DsstinDs2460_delta.pdf","pdf");
  //fitresult->Print("v");
}
