...
TChain chain("h1");
chain.Add("../Ds2317inDs2317_noDsKmvFit.root");

 Float_t pst_dsi, m_dsi, dgf_dsi, c2_dsi, cn_dsi, pst_ds17, m_ds17, dgf_ds17, c2_ds17,  pst_dsii, m_dsii, mc_dsii, dgf_dsii, c2_dsii, cn_dsii, p_pi0, dgf_pi0, c2_pi0, dgf_phi1, c2_phi1, dgf_phi2, c2_phi2, c2_ipi, dgf_ipi, p_ipi, c2_iipi, dgf_iipi, p_iipi, c2_kst0, dgf_kst0, p_kst0, k1_id, k2_id, k3_id, k4_id, pi1_id, pi2_id, mc_ds17, mc_dsi, cosgg, hel_phi, hel_ds17;
...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(m_dsii>sgnl && m_dsii<sgnr && (m_ds17-m_dsii)>range0 && (m_ds17-m_dsii)<range1 && TMath::Abs(hel_phi)>0.35 && hel_ds17<0.7 && TMath::Prob(c2_ds17, dgf_ds17)>0.001 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2){ 
    mass->setVal(m_ds17-m_dsii);  
    data->add(RooArgSet(*mass));}
  if(((m_dsii>lsbl && m_dsii<lsbr) || (m_dsii>rsbl && m_dsii<rsbr)) && (m_ds17-m_dsii)>range0 && (m_ds17-m_dsii)<range1 && TMath::Abs(hel_phi)>0.35 && hel_ds17<0.7 && TMath::Prob(c2_ds17, dgf_ds17)>0.001 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2){
    mass->setVal(m_ds17-m_dsii);
  ...
  */
  cv->SaveAs("SMC_sb_Ds2317_ready.pdf","pdf");
