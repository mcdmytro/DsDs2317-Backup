...
Float_t pst_phi1;
chain.SetBranchAddress("pst_phi1", &pst_phi1);

RooDataSet *data = new RooDataSet("data","", RooArgSet(*x),"GeV");

for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(pst_phi1<range1 && pst_phi1>range0){
    x->setVal(pst_phi1);
...
cv->SaveAs("../pdf_files/phi_pst_reco.pdf","pdf");
cv->SaveAs("../eps_files/phi_pst_reco.eps","eps");
}
