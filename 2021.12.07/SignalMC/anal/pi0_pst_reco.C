...
Float_t pst_pi0;
chain.SetBranchAddress("pst_pi0", &pst_pi0);

RooDataSet *data = new RooDataSet("data","", RooArgSet(*x),"GeV");

for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(pst_pi0<range1 && pst_pi0>range0){
    x->setVal(pst_pi0);
...
frame->GetYaxis()->SetTitle("Events/4 MeV/c");
...
cv->SaveAs("../pdf_files/pi0_pst_reco.pdf","pdf");
cv->SaveAs("../eps_files/pi0_pst_reco.eps","eps");
}
