...
void phi_energy_reco(){
gROOT->Reset();
gSystem->Load("libRooFit");

Float_t range0=1.0, range1=1.9;
...
Float_t e_phi1;
chain.SetBranchAddress("e_phi1", &e_phi1);

RooDataSet *data = new RooDataSet("data","", RooArgSet(*x),"GeV");

for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(e_phi1<range1 && e_phi1>range0){
    x->setVal(e_phi1);
...
cv->SaveAs("../pdf_files/phi_energy_reco.pdf","pdf");
cv->SaveAs("../eps_files/phi_energy_reco.eps","eps");
}
