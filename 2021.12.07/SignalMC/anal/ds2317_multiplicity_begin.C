...
void ds2317_multiplicity_begin(){
gROOT->Reset();
gSystem->Load("libRooFit");

Float_t range0=-1, range1=20;
...
Float_t mult_b;
chain.SetBranchAddress("mult_b", &mult_b);

RooDataSet *data = new RooDataSet("data","", RooArgSet(*x));

for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(mult_b>x0 && mult_b<x1){
    x->setVal(mult_b);
...
cv->SaveAs("./pdf_files/SMC_ds2317_multiplicity_begin.pdf","pdf");
}
