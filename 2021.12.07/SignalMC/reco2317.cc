...
//Added some comments
  ...
  void User_reco::hist_def( void )
  {
    ...
    std::string rs = \
      t_par("DsP")+t_par("Ds17")+t_par("DsI")+t_par("DsII")+t_par("phi1")+t_par("phi2")+t_par("pi1")+t_par("pi2")+t_par("ipi")+t_par("iipi")+t_par("kst1")+t_par("kst2")+t_par("pi0")+t_par("K1")+t_par("K2")+t_par("K3")+t_par("K4")+ \
    "egam1 egam2 asym egam3 egam4 egam5 egam6 ip_x ip_y ip_z mctr cos0 thrr thrs r2 DsPart count \
    dsp_gm dsp_ge dsp_gp dsp_gps ds17_gm ds17_ge ds17_gp ds17_gps ds_gm ds_ge ds_gp ds_gp phi_gm phi_ge phi_gp phi_gpst pi0_gm pi0_ge pi0_gp pi0_gpst \
    k1_id k2_id pi1_id k3_id k4_id pi2_id \
  ...
  void User_reco::event ( BelleEvent* evptr, int* status ) {
    ...
    */
    ...
    double minegam=0.10;
    if(pi0_chi2<200 && pi0_asym>0.0 && ppi0>0.150 && egam0>minegam && egam1>minegam){
    new_pi0.push_back(pi0_fitted);
...
makeKvFit(kst0, 1);
makeKvFit(bkst0, 1);
//withMassCut(kst0, 0.8, 1.0);
//withMassCut(bkst0, 0.8, 1.0);
...
//float m_DspS=Ptype(10431).mass();
//printf("Ds(2317) mass=%f\n",(float)Ptype(10431).mass());
//printf("Ds(2460) mass=%f\n",(float)Ptype(20433).mass());
  double DspS_w=0.04;
  p_list DSPS;
 ...
 */
 ...
 for(unsigned i=0;i<DSPRT.size();++i){
   if (isMC) {
     ...
     t1->column("dsp_gm",Dsprt.m());
     t1->column("dsp_gp",Dsprt.vect().mag());
     t1->column("dsp_ge",Dsprt.e());
     t1->column("dsp_gps",pStar(Dsprt,BeamEnergy::E_HER(),BeamEnergy::E_LER(),BeamEnergy::Cross_angle()).vect().mag());
...
//===================================================================================
  ...
  int makeKmvFit(Particle &p, double mass) {
    if(p.nChildren()<2) return 1;
    if(! &p.userInfo() ) setUserInfo(p);
    dynamic_cast<UserInfo&>(p.userInfo()).vmass(p.mass());
    double v_chi=-1;
  ...
  void makeKmvFit(vector<Particle> &plist, double mass) {
    for(unsigned i=0;i<plist.size();++i) if (makeKmvFit(plist[i]), mass) {plist.erase(plist.begin()+i);--i;}
  }
...
//===================================================================================
  ...
  int dsii_cn=info_dsii.channel();
    ...
    switch (dsii_cn){
        ...
        case 3:{
          t_fill(p.child(0).child(0).child(0),t,"kst2");
          t_fill(p.child(0).child(0).child(0).child(0),t,"K1");
    ...
    switch (dsi_cn){
        ...
        case 3:{
          t_fill(p.child(1).child(0),t,"kst1");
          t_fill(p.child(1).child(0).child(0),t,"K3");
