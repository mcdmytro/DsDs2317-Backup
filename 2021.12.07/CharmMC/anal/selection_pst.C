...
void selection_pst(){
  ...
  for(Float_t pst=r0; pst<=r1; pst+=0.01){
    int s=chain.GetEntries(Form("pst_ds17>%f && cn_dsi!=2 && cn_dsii!=2 && TMath::Abs(mc_ds17)==10431", pst));
    int b=chain.GetEntries(Form("pst_ds17>%f && cn_dsi!=2 && cn_dsii!=2", pst));
  ...
  cv->SaveAs("CMC_selection_pst.pdf","pdf");
}
