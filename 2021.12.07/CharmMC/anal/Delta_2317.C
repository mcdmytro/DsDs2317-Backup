...
void Delta_2317(){
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if(TMath::Abs(mc_ds17)!=10431 && (m_ds17-m_dsii)>range0 && (m_ds17-m_dsii)<range1 && pst_ds17>2.79 && TMath::Abs(hel_phi)>0.44 && TMath::Prob(c2_ds17, dgf_ds17)>0.001 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2){
  ...
  RooRealVar *sig  = new RooRealVar("N_sig",  "N true signal", 0, 5E4);
  RooRealVar *refl = new RooRealVar("N_refl", "N refl signal", 0, 5E4);
  RooRealVar *BS   = new RooRealVar("N_BS",   "N broken signal", 0, 5E4);
  RooRealVar *bkg  = new RooRealVar("N_bkg",  "N background", 0, 1E6);
  ...
  RooAddPdf *pdf = new RooAddPdf ("pdf", "Gaussian + Pol", RooArgList(*gausR, *Pol), RooArgList(*refl, *bkg));
  ...
  cv->SaveAs("CMC_Ds2317_delta_notTrue_referrence.pdf","pdf");
  fitresult->Print("v");
}
