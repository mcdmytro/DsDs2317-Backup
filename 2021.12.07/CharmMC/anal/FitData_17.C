...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(m_ds17>range0 && m_ds17<range1 && pst_ds17>2.83 && TMath::Abs(hel_phi)>0.47 && TMath::Prob(c2_ds17, dgf_ds17)>0.001 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2){
    mass->setVal(m_ds17);
 ...
 */
 ...
 //std::cout<<"chi^2 = "<< frame->chiSquare() <<std::endl;
  cv->SaveAs("CMC_Ds2317_optimized_pstANDhel.pdf","pdf");
}
