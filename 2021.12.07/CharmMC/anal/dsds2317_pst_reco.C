...
#65;6200;1cinclude "RooGaussian.h"
...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(pst_dsp<range1 && pst_dsp>range0 && (TMath::Abs(mc_dsii)!=431 || TMath::Abs(mc_ds17)!=10431) && pst_ds17>3.5 && TMath::Abs(hel_phi)>0.35 && TMath::Prob(c2_ds17, dgf_ds17)>0.001 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2){
    x->setVal(pst_dsp);
...
cv->SaveAs("CMC_dsds2317_pst_bkg.pdf","pdf");
