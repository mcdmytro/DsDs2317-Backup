...
Float_t range0=4.5, range1=10;
Float_t x0=2.215, x1=2.45;
Float_t width=0.008;
RooRealVar *mass = new RooRealVar("mass","",range0,range1);
TChain chain("h1");
chain.Add("../Ds2317_Dsp.root");

 Float_t m_dsp, mctruth, cos, R2, hel_ds17, hel_phi, pst_dsi, m_dsi, dgf_dsi, c2_dsi, cn_dsi, pst_ds17, m_ds17, mc_ds17, dgf_ds17, c2_ds17, cn_ds17, pst_dsii, m_dsii, dgf_dsii, c2_dsii, cn_dsii, ppi, dgf_pi0, c2_pi0, dgf_phi1, c2_phi1, dgf_phi2, c2_phi2, c2_ipi, dgf_ipi, pipi, c2_iipi, dgf_iipi, piipi, c2_kst, dgf_kst, pkst;
...
chain.SetBranchAddress("c2_kst0", &c2_kst);
...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if((m_dsp-0.1922)>range0 && (m_dsp-0.1922)<range1 && pst_ds17>3.5 && pst_dsii>2.5 && TMath::Abs(hel_phi)>0.35 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2)){
    mass->setVal(m_dsp-0.1922);
 ...
 */
 ...
 cv->SaveAs("SMC_DspinDs2317.pdf","pdf");
  fitresult->Print("v");
}
