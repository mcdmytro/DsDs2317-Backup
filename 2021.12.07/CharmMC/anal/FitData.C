...
chain.Add("../Ds2317_noDsKmvFit.root");
 ...
 Float_t r2, m_dsp, pst_dsp, mctruth, pst_dsi, m_dsi, mc_dsi, dgf_dsi, c2_dsi, cn_dsi, pst_ds17, m_ds17, dgf_ds17, c2_ds17,pst_dsii, m_dsii, dgf_dsii, c2_dsii, cn_dsii, ppi, dgf_pi0, c2_pi0, dgf_phi1, c2_phi1, dgf_phi2, c2_phi2, c2_ipi, dgf_ipi, pipi, c2_iipi, dgf_iipi, piipi, mc_ds17, mc_dsi, hel_phi;
...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(m_dsii>range0 && m_dsii<range1 && pst_dsp/sqrt(10.56*10.56-m_dsp*m_dsp)>0.03 && TMath::Abs(hel_phi)>0.35 && TMath::Prob(c2_ds17, dgf_ds17)>0.001 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2){// && TMath::Abs(mc_dsi)==431){
    mass->setVal(m_dsii);
  ...
  */
  ...
  cv->SaveAs("CMC_Ds_optimized_xp.pdf","pdf");
}
