...
chain1.Add("../../Data/Ds2317.root");
Float_t m_dsp,   hel_phi,   pst_ds17,   m_ds17,   m_dsii,   dgf_dsii,   c2_dsii,   cn_dsii,   dgf_pi0,   c2_pi0;
Float_t m_dsp_1, hel_phi_1, pst_ds17_1, m_ds17_1, m_dsii_1, dgf_dsii_1, c2_dsii_1, cn_dsii_1, dgf_pi0_1, c2_pi0_1;
...
for (int i=0; i<chain1.GetEntries();i++){
  ...
  chain1.GetEntry(i);
  if(m_var>range0 && m_var<range1 && m_ds17_1>x0_ds17 && m_ds17_1<x1_ds17 && pst_ds17_1>3.5 && TMath::Abs(hel_phi_1)>0.35 && TMath::Prob(c2_dsii_1, dgf_dsii_1)>0.001 && TMath::Prob(c2_pi0_1, dgf_pi0_1)>0.01 &&\
     cn_dsii_1!=2){
  mass->setVal(m_var);
  data1->add(RooArgSet(*mass));
}}

 RooDataHist *datah = new RooDataHist("datah", "binned version of data", RooArgSet(*mass), *data);
 ...
 data->plotOn(frame, Rescale(0.0046));
 data1->plotOn(frame, Rescale(0.035), MarkerColor(kGreen+3));
 ...
 */
 ...
 cv->SaveAs("CMC_DspinDs2317_subtracted.pdf","pdf");
  fitresult->Print("v");
}
