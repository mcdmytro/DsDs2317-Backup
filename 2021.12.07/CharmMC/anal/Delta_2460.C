...
void Delta_2460(){
  ...
  //Float_t x0=0.326, x1=0.366;
  Float_t x0=0.25, x1=0.55;
  RooRealVar *mass = new RooRealVar("mass","",range0,range1);
  TChain chain("h1");
  chain.Add("../Ds2460.root");

  Float_t m_dsw, pst_dsw, dgf_dsw, c2_dsw, cn_dsw, m_dsn, pst_dsn, dgf_dsn, c2_dsn, cn_dsn, m_ds60, mc_ds60, pst_ds60, dgf_ds60, c2_ds60, cn_ds60, p_pi0, dgf_pi0, c2_pi0, m_phin, c2_phin, dgf_phin, c2_phiw, dgf_phiw, c2_pi0n, dgf_pi0n, p_pi0n, c2_pi0w, dgf_pi0w, p_pi0w, m_kstn, c2_kstn, dgf_kstn, p_kstn, m_dsst, pst_dsst, dgf_dsst, c2_dsst, mctr, pstg_dss, hel_phi, hel_ds60, cos60ds;
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if(abs(mc_ds60)!=20433 && (m_ds60-m_dsst)>range0 && (m_ds60-m_dsst)<range1 && pst_ds60>3.5 && TMath::Abs(hel_phi)>0.35 && cn_dsn!=2 && TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001){
  ...
  RooRealVar *Npol_bkg  = new RooRealVar("Npol_bkg",  "N background", 0, 1E5);
  ...
  RooAddPdf *pdf = new RooAddPdf ("pdf", "2 bkg Gaussian + Pol", RooArgList(*gausBS, *gausR, *Pol), RooArgList(*frac1, *frac2));
  ...
  cv->SaveAs("CMC_Ds2460_notTrue_frac.pdf","pdf");
  fitresult->Print("v");
}
