...
namespace Belle {
  ...
  void User_reco::hist_def( void )
  {
    ...
    std::string rs = t_par("DsP")+t_par("Ds17")+t_par("DsI")+t_par("DsII")+t_par("phi1")+t_par("phi2")+t_par("pi1")+t_par("pi2")+t_par("ipi")+t_par("iipi")+t_par("kst0")+t_par("pi0")+t_par("K1")+t_par("K2")+t_par("K3")+t_par("K4")+"egam1 egam2 asym egam3 egam4 egam5 egam6 ip_x ip_y ip_z mctr cos0 thrr thrs r2 DsPart count ds17_gm ds17_ge ds17_gp ds17_gps ds_gm ds_ge ds_gp ds_gp phi_gm phi_ge phi_gp phi_gpst pi0_gm pi0_ge pi0_gp pi0_gpst cosgg cosdspi0 cos17ds hel_phi hel_ds hel_ds17 k1_id k2_id pi1_id k3_id k4_id pi2_id x_p ecm mult_b mult_e ds_moth";
    // std::string rs2="pi0_m egam chi2 dgf asym ppi0 ";
    //t0=btm->ntuple("0", rs2);
    t1=btm->ntuple("1",rs);
    // t2=btm->ntuple("second","mult_b mult_e");
    //t3=btm->ntuple("3", "PID");
  ...
  void User_reco::event ( BelleEvent* evptr, int* status ) {
    ...
    */
    ...
    double minegam=0.03;
    if(pi0_chi2<20000 && pi0_asym<0.9 && ppi0>0.01 && egam0>minegam && egam1>minegam){
    new_pi0.push_back(pi0_fitted);
    makeKvFit(pi0_fitted,1);
    //UserInfo & info =dynamic_cast<UserInfo&>(pi0_fitted.userInfo());
    //double pi0_dgf = info.prob_vchisq();
    //double pi0_m=(pi0_iterator->child(0).p()+pi0_iterator->child(1).p()).m();
    //t0->column("pi0_m", pi0_m);
    //t0->column("egam", egam0);
    //t0->column("chi2", pi0_chi2);
    //t0->column("dgf", pi0_dgf);
...
//Get particles info
  ...
  //*************** Make Ds(2317)+ particles ****************************
    ...
    setUserInfo(DSPS,1);
    makeKvFit(DSPS,1);
    // withMassCut(DSPS, 2.1, 2.5);
    //withMassCut(DSPS, 2.305, 2.329);
    //makeKmvFit(DSPS);
  ...
  //*************** Make Ds(2317)- particles ****************************
    ...
    setUserInfo(DSMS,1);
    makeKvFit(DSMS,1);
    // withMassCut(DSMS, 2.1, 2.5);
    //withMassCut(DSMS, 2.305, 2.329);
    //makeKmvFit(DSMS);
  ...
  for(unsigned i=0;i<DSPS.size();++i)
    if(pStar(DSPS[i],BeamEnergy::E_HER(),BeamEnergy::E_LER(),BeamEnergy::Cross_angle()).vect().mag()<2.79){DSPS.erase(DSPS.begin()+i);--i;}
  for(unsigned i=0;i<DSMS.size();++i)
    if(pStar(DSMS[i],BeamEnergy::E_HER(),BeamEnergy::E_LER(),BeamEnergy::Cross_angle()).vect().mag()<2.79){DSMS.erase(DSMS.begin()+i);--i;}
  */
  //*************** Make Dsp1 particles ****************************
 ...
 */
 ...
 int bst=0;
 double del_min=0;
 if((int)DSPRT.size()!=0) del_min = (DSPRT[0].child(0).momentum().p().mag()-2.317)*(DSPRT[0].child(0).momentum().p().mag()-2.317) + (DSPRT[0].child(1).momentum().p().mag()-1.969)*(DSPRT[0].child(1).momentum().p().mag()-1.969);
 if((int)DSPRT.size()>1){
   for (int i=1; i<(int)DSPRT.size(); ++i){
 ...
 */
 ...
 if((int)DSPRT.size()>1){
   for(int i=1; i<(int)DSPRT.size(); ++i){
     UserInfo & info =dynamic_cast<UserInfo&>(DSPRT[i].userInfo());
     if(info.vchisq()<min){
       min = info.vchisq();
       bst = i;
   }}}
 ...
 if(DSPRT.size()!=0)
   t1->column("mult_e", DSPRT.size());

 Hep3Vector  CMBoost   = - BeamEnergy::CMBoost();
 for(int i=0;i<(int)DSPRT.size();++i){
   if (isMC) {
     ...
     for(std::vector<Gen_hepevt>::iterator it = hepevt_mag.begin(); it!=hepevt_mag.end(); ++it){
       //Gen_hepevt& particle = *it;
       	 t1->column("phi_gen",phi.momentum().mass());
     }}

   std::vector<Hep3Vector> sVic;
   for (int j=0; j < (int)DSPRT[i].relation().nFinalStateParticles();++j){
   ...
   // ***** x_p variable *****
   double pst_dsp = pStar(DSPRT[i],BeamEnergy::E_HER(),BeamEnergy::E_LER(),BeamEnergy::Cross_angle()).vect().mag();
   double m_dsp   = DSPRT[i].momentum().mass();
   double x_p     = pst_dsp/sqrt(ecm*ecm-m_dsp*m_dsp);
   t1->column("x_p", x_p);
   t1->column("ecm", ecm);

   // ***** Define MCTruth condition *****
   ...
   int ds_moth;
   if(abs(Get.MiD(DSPRT[i].child(0).child(0).child(0),1))==433) ds_moth=1;
   else if(abs(Get.MiD(DSPRT[i].child(0).child(0).child(0),1))==10431) ds_moth=2;
   else ds_moth=-1;
   t1->column("ds_moth", ds_moth);
...
void makeKvFit(vector<Particle> &plist, int flg) {
  for(int i=0;i<(int)plist.size();++i) if(makeKvFit(plist[i], flg)) {plist.erase(plist.begin()+i);--i;}
}
...
void makeKmvFit(vector<Particle> &plist) {
  for(int i=0;i<(int)plist.size();++i) if (makeKmvFit(plist[i])) {plist.erase(plist.begin()+i);--i;}
}
...
void User_reco::t_fill(Particle &p,BelleTuple *t,std::string tg){
  ...
  if(isMC){t->column(vals[9]+tag,IDhep(p));}
...
//===================================================================================
  ...
  int dsii_cn=info_dsii.channel();
    ...
    atc_pid sel_kpi(3,1,5,3,2);
    float k1_id=sel_kpi.prob(& (p.child(0).child(0).child(0).child(0).mdstCharged()));
    float k2_id=sel_kpi.prob(& (p.child(0).child(0).child(0).child(1).mdstCharged()));
    float pi1_id=sel_kpi.prob(& (p.child(0).child(0).child(1).mdstCharged()));
    float k3_id=sel_kpi.prob(& (p.child(1).child(0).child(0).mdstCharged()));
    float k4_id=sel_kpi.prob(& (p.child(1).child(0).child(1).mdstCharged()));
    float pi2_id=sel_kpi.prob(& (p.child(1).child(1).mdstCharged()));
    t->column("k1_id", k1_id);
    t->column("k2_id", k2_id);
    t->column("pi1_id", pi1_id);
    t->column("k3_id", k3_id);
    t->column("k4_id", k4_id);
    t->column("pi2_id", pi2_id);
    t->dumpData();
  }
...
void withDrDz(p_list &plist, double maxdr, double maxdz){
  for(int i=0;i<(int)plist.size();++i) if(withDrDz(plist[i], maxdr,maxdz)) {plist.erase(plist.begin()+i);--i;}
    }
...
void combination5p(p_list &plist, const Ptype &ptype, p_list &p1, p_list &p2, p_list &p3, p_list &p4, p_list &p5){
  for(long int i=0;i<(int)p1.size();++i)
    for(long int j=0;j<(int)p2.size();++j)
      for(long int k=0;k<(int)p3.size();++k)
	for(long int l=0;l<(int)p4.size();++l)
	  for(long int m=0;m<(int)p5.size();++m)
...
void combination6p(p_list &plist, const Ptype &ptype, p_list &p1, p_list &p2, p_list &p3, p_list &p4, p_list &p5, p_list &p6){
  for(long int i=0;i<(int)p1.size();++i)
	...
	for(long int l=0;l<(int)p4.size();++l)
	  for(long int m=0;m<(int)p5.size();++m)
	    for(long int n=0;n<(int)p6.size();++n)
