...
namespace Belle {
  ...
  void User_reco::hist_def( void )
  {
    ...
    std::string rs = \
    t_par("mp")+t_par("ds60")+t_par("dsst")+t_par("dsw")+t_par("dsn")+t_par("phiw")+t_par("phin")+t_par("pi0w")+t_par("pi0n")+t_par("kstn")+t_par("kstw")+t_par("pi0")+t_par("k1w")+t_par("k2w")+t_par("k1n")+t_par("k2n")+t_par("piw")+t_par("pin")+ \
...
//*************** Make Ds* particles ******************************
  ...
  combination(DsstP, Ptype(433), DsPn, gamma, Dsst_w*10);
  setUserInfo(DsstP, 1);
  makeKvFit(DsstP, 1);
  //withMassCut(DsstP, 2.0, 2.25);
  withMassCut(DsstP, 2.097, 2.127);
  makeKmvFit(DsstP);
  if(isMC) {setGenHepInfoT(DsstP);}

  combination(DsstM, Ptype(-433), DsMn, gamma, Dsst_w*10);
  setUserInfo(DsstM, 1);
  makeKvFit(DsstM, 1);
  //withMassCut(DsstM, 2.0, 2.25);
...
//BCS
 int bst=0;
 double del_min=0;
 if((int)DSPRT.size()!=0) del_min = (DSPRT[0].child(0).momentum().p().mag()-2.460)*(DSPRT[0].child(0).momentum().p().mag()-2.460) + (DSPRT[0].child(1).momentum().p().mag()-1.969)*(DSPRT[0].child(1).momentum().p().mag()-1.969);
 if((int)DSPRT.size()>1){
   for (int i=1; i<(int)DSPRT.size(); ++i){
     if ((DSPRT[i].child(0).momentum().p().mag()-2.460)*(DSPRT[i].child(0).momentum().p().mag()-2.460) +\
	 (DSPRT[i].child(1).momentum().p().mag()-1.969)*(DSPRT[i].child(1).momentum().p().mag()-1.969)<del_min){
       del_min=(DSPRT[i].child(0).momentum().p().mag()-2.460)*(DSPRT[i].child(0).momentum().p().mag()-2.460) +\
...
// ***** Defane MCTruth condition *****
   ...
   t1->column("ds_moth", abs(Get.MiD(DSPRT[i].child(0).child(0).child(0),1)));
...
// mp ds60 dsst dsw dsn phiw phin pi0w pi0n kst0 pi0 k1w k2w k1n k2n piw pin egam_dss egam_dsj0 egam_dsj1 egam_n0 egam_n1 egam_w0 egam_w1
  ...
  int dsw_cn = info_dsw.channel();
    ...
    switch (dsn_cn){
        ...
        case 3:{
          t_fill(p.child(0).child(0).child(0).child(0),t,"kstn");
          t_fill(p.child(0).child(0).child(0).child(0).child(0),t,"k1n");
    ...
    switch (dsw_cn){
        case 1:{
          t_fill(p.child(1).child(0),t,"phiw");
        ...
        case 3:{
          t_fill(p.child(1).child(0),t,"kstw");
          t_fill(p.child(1).child(0).child(0),t,"k1w");
