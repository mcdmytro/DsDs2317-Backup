...
void TMVAApplication_Data(){
  ...
  chain->Add("Data_Ds2317_forTMVA.root");
  ...
  TNtuple* out  = new TNtuple("out",  "Out tree", "mvaValue:dm_ds17");

  for(Long64_t ievt=0; ievt<chain->GetEntries(); ievt++){
      chain->GetEntry(ievt);
      Double_t mvaValue = reader->EvaluateMVA("MLP classifier");
      out->Fill(mvaValue, dm_ds17);
    }

  TFile *MyFile = new TFile("Data_outTMVA.root","RECREATE");
  out ->Write();
  MyFile->Close();
