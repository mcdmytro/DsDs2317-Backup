...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(m_dsii>range0 && m_dsii<range1 && TMath::Abs(hel_phi)>0.35 && TMath::Prob(c2_ds17, dgf_ds17)>0.001 && TMath::Prob(c2_dsii, dgf_dsii)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && cn_dsii!=2){// && TMath::Abs(mc_dsi)==431){
    mass->setVal(m_dsii);
  ...
  */
  ...
  cv->SaveAs("SMC_SB_Ds_ready.pdf","pdf");
}
