...
void ds2317_momentum_reco(){
gROOT->Reset();
gSystem->Load("libRooFit");

Float_t range0=0.5, range1=1.6;
...
Float_t p_ds17;
chain.SetBranchAddress("p_ds17", &p_ds17);

RooDataSet *data = new RooDataSet("data","", RooArgSet(*x),"GeV");

for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(p_ds17<range1 && p_ds17>range0){
    x->setVal(p_ds17);
...
frame->GetYaxis()->SetTitle("Events/11 MeV/c");
...
cv->SaveAs("../pdf_files/ds2317_momentum_reco.pdf","pdf");
cv->SaveAs("../eps_files/ds2317_momentum_reco.eps","eps");
