...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(m_phi2>range0 && m_phi2<range1){// && mc_phi2==333){
    mass->setVal(m_phi2);
...
*/
RooRealVar *mean = new RooRealVar("mean", "Mean of gaussian",1.01946,1.010,1.030,"GeV");
//RooRealVar *mean = new RooRealVar("mean", "Mean of gaussian",2.317,2.28,2.36,"GeV");
  ...
  RooRealVar *mZ=new RooRealVar("mean", "Mean of Voigt",1.01946,1.018,1.022,"GeV");
  RooRealVar *GZ=new RooRealVar("gamma", "Gamma of Voigt",0.00464,0.003,0.015,"GeV");
  RooRealVar *sigmaV = new RooRealVar("sigma", "Sigma of Voigt",0.00057,0.0001,0.001,"GeV");
  RooVoigtian *voigt = new RooVoigtian("voigt","Reconstructed peak", *mass, *mZ, *GZ, *sigmaV);
  ...
  RooAddPdf *pdf = new RooAddPdf ("pdf", "Gaussian + Pol",RooArgList(*voigt, *Pol), RooArgList(*sig, *bkg));
  ...
  cv->SaveAs("noPID_noMCtruth.pdf","pdf");
}
