...
chain.Add("../Ds2317.root");
 ...
 Float_t mctruth, cos, R2, hel_phi,pst_dsi, m_dsi, dgf_dsi, chi2_dsi, cn_dsi, pst_ds17, m_ds17, dgf_ds17, chi2_ds17,pst_dsii, m_dsii, dgf_dsii, chi2_dsii, cn_dsii, ppi, dgf_pi, chi2_pi, dgf_phi1, chi2_phi1, dgf_phi2, chi2_phi2, chi2_ipi, dgf_ipi, pipi, chi2_iipi, dgf_iipi, piipi, chi2_kst, dgf_kst, pkst;
...
for (int i=0; i<chain.GetEntries();i++){
  chain.GetEntry(i);
  if(m_ds17>range0 && m_ds17<range1 && TMath::Abs(hel_phi)>0.35 && pst_ds17>3.5 && TMath::Prob(chi2_ds17, dgf_ds17)>0.001 && TMath::Prob(chi2_dsii, dgf_dsii)>0.001 && TMath::Prob(chi2_pi, dgf_pi)>0.01 && ((cn_dsii!=2) || (cn_dsii==2 && TMath::Prob(chi2_iipi, dgf_iipi)>0.1 && TMath::Prob(chi2_kst,dgf_kst)>0.01 && piipi>0.3))){
  mass->setVal(m_ds17);
