...
void Fit_Dsstar_mass(){
  ...
  Float_t x0=2.075, x1=2.185;
  RooRealVar *mass = new RooRealVar("mass","",range0,range1);
  TChain chain("h1");
  chain.Add("../Ds2460_noDsstKmvFit.root");

  Float_t m_dsw, pst_dsw, dgf_dsw, c2_dsw, cn_dsw, m_dsn, pst_dsn, dgf_dsn, c2_dsn, cn_dsn, m_ds60, mc_ds60, pst_ds60, dgf_ds60, c2_ds60, cn_ds60, p_pi0, dgf_pi0, c2_pi0, c2_phin, dgf_phin, c2_phiw, dgf_phiw, c2_pi0n, dgf_pi0n, p_pi0n, c2_pi0w, dgf_pi0w, p_pi0w, c2_kst0, dgf_kst0, p_kst0, mctr, pstg_dss, hel_phi, hel_ds60, cn_dsn, cos60ds, m_dsst, c2_dsst, dgf_dsst; 
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if(m_dsst>range0 && m_dsst<range1 && pst_ds60>3.5 && pst_dsn>2.5 && TMath::Abs(hel_phi)>0.35 && cn_dsn!=2 & TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001){
      mass->setVal(m_dsst);
      data->add(RooArgSet(*mass));
    }}
  //TMath::Abs(mc_ds60)==20433 
  //TMath::Abs(mc_ds60)!=20433  && m_ds60>range0 && m_ds60<range1 && pst_ds60>3.5 && pst_dsn>2.5 && TMath::Abs(hel_phi)>0.35 && cn_dsn!=2 && TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001
  RooRealVar *mean  = new RooRealVar("mean", "sgn mean", 2.112, 2.102, 2.122);
  ...
  RooRealVar *bkg = new RooRealVar("Npol bkg", "", 0., 1E6); 
  ...
  */
  ...
  cv->SaveAs("Data_Dsst_sb_ready.pdf","pdf");
  fitresult->Print("v");
}
