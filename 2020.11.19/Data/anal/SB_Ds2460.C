...
for (int i=0; i<chain.GetEntries();i++){
  ...
  if(m_dsst<2.127 && m_dsst>2.097 && (m_ds60-m_dsst)>range0 && (m_ds60-m_dsst)<range1 && pst_ds60>3.5 && pstg_dss>0.1 && TMath::Abs(hel_phi)>0.35 && cn_dsn!=2 && TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001){ 
    mass->setVal(m_ds60-m_dsst);  
    data->add(RooArgSet(*mass));}
  if(m_dsst<2.185 && m_dsst>2.1245 && (m_ds60-m_dsst)>range0 && (m_ds60-m_dsst)<range1 && pst_ds60>3.5 && pstg_dss>0.1 && TMath::Abs(hel_phi)>0.35 && cn_dsn!=2 && TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001){
  mass->setVal(m_ds60-m_dsst);
...
//m_ds60>range0 && m_ds60<range1 && pst_ds60>3.5 && pst_dsn>2.5 && TMath::Abs(hel_phi)>0.35 && cn_dsn!=2 && TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001
  ...
  */
  cv->SaveAs("Data_sb_Ds2460_ready.pdf","pdf");
