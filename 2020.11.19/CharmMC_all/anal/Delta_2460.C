...
void Delta_2460(){
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if(TMath::Abs(mc_ds60)==20433 && (m_ds60-m_dsst)>range0 && (m_ds60-m_dsst)<range1 && pst_ds60>3.5 && TMath::Abs(hel_phi)>0.35 && cn_dsn!=2 && TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001){
  ...
  */
  ...
  cv->SaveAs("CMC_Ds2460_delta_True_ref2.pdf","pdf");
  fitresult->Print("v");
}
