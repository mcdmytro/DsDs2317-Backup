...
void Fit_Ds2460_mass(){
  ...
  Float_t range0=2.25, range1=2.7;
  //Float_t x0=2.32, x1=2.57;
  //Float_t x0=2.44, x1=2.48;
  Float_t x0=2.32, x1=2.65;
  RooRealVar *mass = new RooRealVar("mass","",range0,range1);
  TChain chain("h1");
  chain.Add("../Ds2460.root");

  Float_t m_dsw, pst_dsw, dgf_dsw, c2_dsw, cn_dsw, m_dsn, pst_dsn, dgf_dsn, c2_dsn, cn_dsn, cn_dsw, m_ds60, mc_ds60, pst_ds60, dgf_ds60, c2_ds60, cn_ds60, p_pi0, dgf_pi0, c2_pi0, m_phin, c2_phin, dgf_phin, c2_phiw, dgf_phiw, c2_pi0n, dgf_pi0n, p_pi0n, c2_pi0w, dgf_pi0w, p_pi0w, m_kstn, c2_kstn, dgf_kstn, p_kstn, m_dsst, pst_dsst, dgf_dsst, c2_dsst, mctr, pstg_dss, hel_phi, hel_ds60, cn_dsn, cos60ds; 
  ...
  chain.SetBranchAddress("m_kstn", &m_kstn);
  ...
  for (int i=0; i<chain.GetEntries();i++){
    chain.GetEntry(i);
    if(m_ds60>range0 && m_ds60<range1 && pst_ds60>3.5 && pst_dsn>2.5 && TMath::Abs(hel_phi)>0.35 && cn_dsn!=2 && TMath::Prob(c2_ds60, dgf_ds60)>0.001 && TMath::Prob(c2_dsn, dgf_dsn)>0.001 && TMath::Prob(c2_pi0, dgf_pi0)>0.01 && TMath::Prob(c2_dsst, dgf_dsst)>0.001){
	mass->setVal(m_ds60);
  ...
  RooRealVar *mean_gBS = new RooRealVar("BS mean",   "Mean of gaussian", 2.4595);//, 2.445, 2.46);
  RooRealVar *mean_gR  = new RooRealVar("Refl mean", "Mean of gaussian", 2.468,  2.45, 2.47);
  RooRealVar *sigma_gS  = new RooRealVar("sgn sigma",  "Sigma of gaussian", 0.0067, 0.003, 0.01);
  ...
  RooRealVar *sig  = new RooRealVar("N_sig",  "N true signal", 1300, 1000, 1600);
  RooRealVar *refl = new RooRealVar("N_refl", "N refl signal", 800, 5E3);
  ...
  //RooAddPdf *pdf = new RooAddPdf ("pdf", "2*Gaussian + Pol", RooArgList(*gausS, *gausBS), RooArgList(*sig, *BS));     
  ...
  */
  ...
  cv->SaveAs("Ds2460inDs2460_mass_ready.pdf","pdf");
  fitresult->Print("v");
}
